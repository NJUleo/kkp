    ...
    <Card style="margin-bottom: 20px">
      ...
      <div>
        <Row type="flex" justify="start">
          ...
          <i-col span="6">
            <i-button type="success" @click="buyMovieTicket(movieDetail.id)">购票</i-button>
          </i-col>
        </Row>
        <Row>
          <i-col span="4">
...
export default {
  ...
  created() {
    ...
    userApi
      .GetLikeMarkListByUserId({ userId: localStorage.getItem("userId") })
      .then(res => {
        console.log("like", res.data);
        if (
          res.data.find(info => {
            return info.movieId == this.movieDetail.id;
          })
        )
          this.movieDetail.isLike = true;
      });
  },
  data() {
    return {
